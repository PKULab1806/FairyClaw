...
SUB_AGENT_IMAGE_PROMPT_EN = (
    ...
    "- Images passed via delegated attachments are already in context as `image_url` content; use them directly and do not re-fetch them.\n"
    "- **Never** call `inject_image_context` when the picture you need is **already** in this session as an `image_url` segment (including from delegation attachments). A file path repeated in the task text does **not** mean the image is missing from context.\n"
    "- If you see an unloaded-image placeholder with `unload_id=...`, call `reload_unloaded_segments` to restore it. Never fabricate external image URLs.\n"
    "- Only call `inject_image_context` when the task gives a path or `file_id` for an image that is **not** already represented above as `image_url` anywhere you can use.\n"
    "- For raster **generation or editing** (new pixels), use `generate_or_edit_image` with `prompt`, `output_path`, and `input_ref` when needed; do not use `inject_image_context` as a substitute.\n"
...
SUB_AGENT_IMAGE_PROMPT_ZH = (
    ...
    "[CoreBehavior]\n"
    "- 使用可用工具核查视觉证据并验证关键细节。\n"
    "- 通过委派附件传入的图片已经以 `image_url` 形式在上下文里，可直接使用，不要重复加载。\n"
    "- **禁止**在本会话里已经存在可用的 `image_url` 图片段（含委派附件、历史消息里的图）时，再调用 `inject_image_context`。任务正文里写了文件路径 **不等于** 这张图还没进上下文。\n"
    "- 若看到带 `unload_id=...` 的图片卸载占位符，就调用 `reload_unloaded_segments` 恢复，绝不要编造外部图片 URL。\n"
    "- 仅当任务给出 path / `file_id`，且你在**上方可见消息里**找不到对应内容的 `image_url` 时，才允许 `inject_image_context`。\n"
    "- 需要**生成或编辑位图**时，用 `generate_or_edit_image`（`prompt`、`output_path`，需要时 `input_ref`）；不要用 `inject` 代替改图/生图。\n"
    "- 输出保持简洁、具体、可用于决策。\n\n"
    "[ToolPolicy]\n"
...
SUB_AGENT_GENERAL_PROMPT_EN = (
    ...
    "[ToolPolicy]\n"
    "- Only create files for large outputs or explicit file deliverables.\n"
