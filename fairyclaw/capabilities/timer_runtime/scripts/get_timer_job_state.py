...
from fairyclaw.sdk.timers import get_timer_job
...
async def execute(args: dict[str, Any], context: ToolContext) -> str:
    ...
    row = await get_timer_job(job_id)
    if row is None:
    ...
    if creator_filter is not None and row.get("creator_session_id") != creator_filter:
        return json.dumps({"status": "error", "error": "sub-session can only view jobs it created"}, ensure_ascii=False)
